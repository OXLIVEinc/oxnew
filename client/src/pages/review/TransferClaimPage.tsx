...
import { Eyebrow, Title, Subtitle, Row, ErrorText, SectionCard, Field, PrimaryButton, GhostButton } from '../../components/review/ui';
    ...
    return (
      <Shell>
        <StubCard
          ...
          bottom={<span className="font-mono text-[13.5px] text-[#1A1A1A]/55">Code: {transfer.transferCode}</span>}
        />
      </Shell>
    );
  }
  ...
  return (
    <Shell>
      <StubCard
        top={
          <>
            ...
            <h1 className="mb-1.5 text-[22px] font-bold leading-tight tracking-tight text-[#1A1A1A]">{ticket.eventName}</h1>
            <p className="text-[13.5px] text-[#1A1A1A]/50">{ticket.tierLabel}</p>
          </>
        }
        bottom={
          <>
      ...
      <SectionCard heading="Claim this ticket">
        <Field id="fullName" label="Full name" value={fullName} placeholder="Full name" onChange={(e) => setFullName(e.target.value)} />
        <Field id="email" type="email" label="Email" value={email} placeholder="name@example.com" onChange={(e) => setEmail(e.target.value)} />
      </SectionCard>
