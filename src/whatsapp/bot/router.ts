...
export async function handleMessage(phone: string, text: string, waName?: string | null): Promise<BotReply> {
  ...
  switch (state) {
    ...
    case 'EVENT_CHECKOUT_PENDING':
      ...
      return {
        ...
        cta: context.checkoutLink
          ? { footerText: 'Expires in 30 minutes', buttonText: 'Complete Checkout', url: context.checkoutLink }
          : undefined,
      };
    ...
    case 'HOTEL_ORDER_PENDING':
      return {
        reply:
          `Still waiting for you to review and complete this booking.\n\n` +
          `${context.paymentLink}\n\n` +
          `Reply HELP if you're having trouble, or PAUSE to set this order aside.`,
        cta: context.paymentLink
          ? { footerText: 'Expires in 30 minutes', buttonText: 'Review & Pay', url: context.paymentLink }
          : undefined,
      };
