...
export const GuestListPanel: React.FC<Props> = ({
  eventId: propEventId,
}) => {
  ...
  const { data: guests, isLoading } = useGuests(selectedEventId, {
    dateFrom,
    dateTo,
  });

  const removeGuest = useRemoveGuest(selectedEventId);
                        ...
                        <td className="px-4 py-3">
                          <button
                            onClick={() => {
                              if (
                                confirm(
                                  "Remove this guest's ticket?"
                                )
                              ) {
                                removeGuest.mutate(g.userId);
                              }
                            }}
