// client/src/components/organizer/AnalyticsPanel.tsx
import React, { useEffect, useState } from "react";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Skeleton } from "@/components/ui/skeleton";
import { useOrganizerEvents, useEventAnalytics } from "@/hooks/api/useOrganizer";
import { formatNaira } from "@/lib/hotelFormat";

interface Props { eventId: string | null; }

export const AnalyticsPanel: React.FC<Props> = ({ eventId: propEventId }) => {
  ...
  useEffect(() => setSelectedEventId(propEventId), [propEventId]);

  const { data, isLoading } = useEventAnalytics(selectedEventId, { dateFrom, dateTo });
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap gap-4 items-end">
        <div className="flex-1 min-w-[200px]">
          ...
          <select value={selectedEventId ?? ""} onChange={(e) => setSelectedEventId(e.target.value || null)} className="w-full border border-border rounded px-3 py-2.5 text-sm bg-background">
        ...
        <div>
          ...
          <input type="date" value={dateFrom} onChange={(e) => setDateFrom(e.target.value)} className="border border-border rounded px-3 py-2.5 text-sm bg-background" />
        </div>
        <div>
          <label className="text-xs font-medium text-muted-foreground block mb-1">To</label>
          <input type="date" value={dateTo} onChange={(e) => setDateTo(e.target.value)} className="border border-border rounded px-3 py-2.5 text-sm bg-background" />
        </div>
      </div>

      {!selectedEventId ? (
        <Card><CardContent className="py-12 text-center text-muted-foreground">Select an event to view analytics</CardContent></Card>
      ) : isLoading || !data ? (
        <div className="grid md:grid-cols-2 gap-4">{Array.from({ length: 4 }).map((_, i) => <Skeleton key={i} className="h-40 w-full" />)}</div>
      ) : (
        <>
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            {[
              ...
              <Card key={c.label}>
                ...
                <CardContent className="text-xl font-semibold">{c.value}</CardContent>
              </Card>
            ))}
          </div>
                ...
                {data.tierBreakdown.map((tier, i) => (
                  <div key={i} className="flex items-center justify-between py-2 border-b border-border last:border-0 text-sm">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <span className="font-medium">{formatNaira(tier.revenue)}</span>
                    </div>
                  </div>
                ))}
              </CardContent>
            ...
            <Card>
              ...
              <CardContent className="space-y-2">
                {Object.entries(data.genderCounts).map(([gender, count]) => (
                  <div key={gender} className="flex items-center justify-between text-sm py-1">
                    <span>{gender}</span><span className="font-medium">{count}</span>
                  </div>
                ))}
              </CardContent>
            </Card>
